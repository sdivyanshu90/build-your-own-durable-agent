"""Frozen SQLAlchemy schema used only by migration revision 0001.

Do not edit this snapshot after release; add a later Alembic revision instead.
"""
...
class SchemaV1Base(DeclarativeBase):
    """Declarative metadata root used by Alembic autogeneration."""


class RunORM(SchemaV1Base):
    __tablename__ = "runs"
...
class PlanRevisionORM(SchemaV1Base):
...
class TaskDependencyORM(SchemaV1Base):
...
class TaskAttemptORM(SchemaV1Base):
...
class CheckpointORM(SchemaV1Base):
...
class ToolCallORM(SchemaV1Base):
...
class ToolResultORM(SchemaV1Base):
...
class ArtifactORM(SchemaV1Base):
...
class EvidenceORM(SchemaV1Base):
...
class ClaimEvidenceLinkORM(SchemaV1Base):
...
class RepositorySnapshotORM(SchemaV1Base):
...
class RepositoryFileORM(SchemaV1Base):
...
class RepositoryChunkORM(SchemaV1Base):
...
class PauseRequestORM(SchemaV1Base):
...
class CancellationRequestORM(SchemaV1Base):
...
class IdempotencyKeyORM(SchemaV1Base):
